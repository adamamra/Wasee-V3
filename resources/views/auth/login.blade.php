            ...
            <div class="p-8">
                <form method="POST" action="{{ route('login') }}">
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" name="login" value="{{ old('login') }}" required autofocus placeholder="example@domain.com أو رقم الهوية"
                            class="w-full px-4 py-3 bg-gray-50 border-2 border-gray-200 rounded-2xl text-gray-700 placeholder-gray-400 focus:border-primary-400 focus:bg-white focus:ring-4 focus:ring-primary-100 transition-all duration-200 outline-none @error('login') border-red-300 bg-red-50 @enderror">
                        @error('login')<span class="text-sm text-red-500 font-medium mt-1.5 block"><i class="fas fa-circle-exclamation ml-1"></i>{{ $message }}</span>@enderror
                    </div>
                    <div class="mb-5">
                        ...
                        @error('password')<span class="text-sm text-red-500 font-medium mt-1.5 block"><i class="fas fa-circle-exclamation ml-1"></i>{{ $message }}</span>@enderror
                    </div>
                ...
                <div class="mt-6 text-center space-y-3">
                    @if (Route::has('organization.login'))
                        <a href="{{ route('organization.login') }}" class="block w-full py-3 text-primary-600 border-2 border-primary-200 hover:border-primary-400 hover:bg-primary-50 rounded-2xl font-bold transition-all duration-200 text-sm"><i class="fas fa-building ml-2"></i>تسجيل دخول المؤسسة</a>
                    @endif
                    ...
                    <a href="{{ route('register') }}" class="block text-primary-600 hover:text-primary-700 font-bold transition-all duration-200 text-sm"><i class="fas fa-user-plus ml-2"></i>إنشاء حساب جديد</a>
                    @if (Route::has('organization.register'))
                        <a href="{{ route('organization.register') }}" class="block text-secondary-600 hover:text-secondary-700 font-bold transition-all duration-200 text-sm"><i class="fas fa-building ml-2"></i>تسجيل مؤسسة جديدة</a>
                    @endif
                </div>
            </div>
        </div>
